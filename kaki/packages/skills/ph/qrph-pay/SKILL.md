...
id: ph.qrph-pay
title: QR Ph payment
when_to_use: Use when the household asks Kaki to handle qr ph payment.
...
2. Use the declared approval surface to gather current data and prepare the task up to the last irreversible action.
3. Stop at the final `money.transfer` boundary with exact evidence; continue only with a scoped, unexpired approval.
...
- Handa na po. I-approve ang huling hakbang; wala pang bayad o booking na ginawa.
